...
CREATE OR ALTER FUNCTION NO_SALE(@item int)
Returns sql_variant
AS 
BEGIN 
	Declare @itemS INT;
	SELECT @itemS = COUNT(StockItemID )
...
WITH CTE AS( SELECT ac.CityName  , COUNT(pl.StockItemId) as Item
 FROM Application.Cities ac
 JOIN Application.StateProvinces ast ON ac.StateProvinceID = ast.StateProvinceID
 JOIN Application.Countries co ON ast.CountryID = co.CountryID and co.CountryID = 230
 JOIN Purchasing.Suppliers ps ON ac.CityID =ps.DeliveryCityID
 JOIN Purchasing.PurchaseOrders pp ON ps.SupplierID = pp.SupplierID and year(pp.OrderDate) =2016
 LEFT JOIN Purchasing.PurchaseOrderLines pl ON pp.PurchaseOrderID = pl.PurchaseOrderID
 GROUP BY ac.CityName
 )

SELECT i.CityName, dbo.NO_SALE(i.Item) AS 'Stock Items'
FROM CTE i
ORDER BY 'Stock Items' DESC
